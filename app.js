  ...
  variants.forEach((vr, idx) => {
    ...
    <div class="result-card ${cardCls}">
      ...
      <div class="calc-chain">
        ...
        <div class="cc-step cc-result">
          <span class="cc-key cc-key--result">Threshold <button class="help-btn" data-tooltip="With ${nVar} variants: 5% error ÷ ${nVar} = ${fmt((5/nVar).toFixed(2),2)}% per variant. Confidence = 100% - ${fmt((5/nVar).toFixed(2),2)}% = ${fmt(adjConf*100,2)}%. You have ${fmt(conf*100,2)}%, need ${fmt(adjConf*100,2)}%." onclick="openConcept('bonferroni')">?</button></span>
